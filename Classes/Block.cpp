...
bool Block::init()
{
			...
			do 
			{
						...
						if((int)_color>0&&(int)_color<=5)
						{
									auto string = _colorStr + 'L';
									m_sprL = ObjectPoolManager::getInstance()->getObject(string);
...
void Block::destroy()
{
			if((int)(this->_color)>0 && (int)(this->_color)<=5&&m_sprL!=nullptr)
			{
						auto string = _colorStr + 'L';
						ObjectPoolManager::getInstance()->pushObject(string, m_sprL);
						removeChild(m_sprL);
						m_sprL = nullptr;
			}
...
void Block::changeLight()
{
			if(_color<=0||_color>5)
			{
						return;
			}
			if(m_sprL!=nullptr && m_sprL->isVisible())
			{
						m_sprL->setVisible(false);
						_spr->setVisible(true);
			}
			else if(m_sprL!=nullptr && ! m_sprL->isVisible())
			{
