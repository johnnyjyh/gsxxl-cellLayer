...
void CellLayer::linkLineInGrid(const Vec2 & source,const Vec2 &dest)
{
			/*Vec2 pos1, pos2;
			...
			auto spr = Sprite::createWithSpriteFrameName("Connection.png");
			
			
		
			//addChild(spr);
			auto temp = spr->getBoundingBox();								
			auto heigth = source.getDistance(dest);
		
		///*	float x = std::abs(dest.x - source.x);
		//	float y = std::abs(dest.y - source.y);
		//	double z = std::sqrt(x*x + y*y);
		//	int rot = std::round((float)(std::asin(x / z) / M_PI * 180));*/

			/*auto func=[](const Vec2 &from,const Vec2 & to){
						...
						float angle = 0;
						if (len_y > 0 && len_x < 0) {
									angle = -std::atan(tan_yx) * 180 / M_PI - 90;
						}
									...
									angle = 90 - std::atan(tan_yx) * 180 / M_PI;
						}
						return angle;
			};*/
			//auto rot = func(source,dest);
			auto rot = (dest - source).getAngle();
			float angle = -(rot) * 180 / 3.14+90;
			...
			Vec2 fixedV(0,0);
			if(source.x>dest.x+30)
			{
						fixedV.x = -heigth / 2;
			}
			else if(source.x<dest.x-30)
			{
						fixedV.x = heigth / 2;
			}

			if(source.y<dest.y-30)
			{
						fixedV.y = heigth / 2;
			}
			else if(source.y>dest.y+30)
			{
						fixedV.y = -heigth / 2;
			}
			...
			spr->setPosition((Vec2(source.x+fixedV.x, source.y+fixedV.y)));
			spr->setScaleY(heigth / temp.size.height);
			addChild(spr, LineZorder::line);
			
				
			
			m_linkLine.push_back(spr);
}

void CellLayer::unLinkLineInGrid()
{
			...
			for (auto &touchlist : _touchCells)
			{
						if (touchlist->getBoundingBox().containsPoint(touch->getLocation()))
						{
									...
									if(touchlist->getLife()>0 && ! (touchlist->_isSelected) && pow(touchlist->getRow() - _touchMoveCells.back()->getRow(), 2) + pow(touchlist->getColumn() - _touchMoveCells.back()->getColumn(), 2)<=2)
									{
												...
												linkLineInGrid(convertToWorldSpace(_touchMoveCells.back()->getPosition()), convertToWorldSpace(touchlist->getPosition()));
